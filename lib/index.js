...
(async function () {
  ...
  function requestPromise(path, method = 'GET', hostname, ops) {
  ...
  function writeConfigFile(configPath, data, message, stopProcess = true) {
    return createWriteStream(configPath)
      ...
      .write(data, 'utf-8', (er) => {
        ...
        return stopProcess ? process.exit(0) : true;
      });
  }
  ...
  async function createIssues(issuesData, config) {
    ...
    const response = await Promise.all(
      [...issuesData].map(async (issueData) => {
        const resultApi = await requestPromise(
          `/repos/${config.gh_username}/${issueData.repoName}/issues`,
          'POST',
          null,
          {
            headers: {
    ...
    if (response.every((res) => !res)) {
      return logMessageError('Oups, something wrong try again..');
    }
  ...
  try {
    ...
    if (argv.commands.length) {
      if (argv.commands.includes('config')) {
        return logMessage('lol config');
      }

      repos = await repoExist(argv.commands, config);
    } else {
      const allRepos = (
    ...
    return createIssues(issueData, config);
  } catch (error) {
    if (error.code === 'ENOENT') {
      ...
      return writeConfigFile(configFilePath, newConfig);
    }
